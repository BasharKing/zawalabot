...
module.exports.run = async (bot, message, args, sql) => {
    ...
    if (talkedRecently.has(message.author.id)) {
        ...
        message.channel.send(timeoute).then(msg => {
            msg.delete(5000)
        });
    } else {
    ...
    ORDER BY a1.xp DESC, a1.UserID DESC) WHERE UserID = '${puser.id}'`).get()
            let sqlstr;
            if (!rows) return message.reply("**:x: | لم يتم تسجيله بقاعدة البيانات بعد**").then(msg => {
                msg.delete(5000)
            });
            ...
            if (rows.length < 1) return message.reply("**:white_check_mark: | تم تسجيلك حاول بعد 20 ثانية**").then(msg => {
                msg.delete(5000)
            });
            let rankedtext = sql.prepare(`SELECT * FROM (SELECT a1.UserID, a1.xp , COUNT (a2.xp) rank 
    FROM profile a1, profile a2 
